...
public class BinarySearch_P1654 {
    static long maxLength(long[] lanCable, int N) {
        long min = 0;
        long max = lanCable[lanCable.length-1];
        long middle = 0;
        while (min <= max) {
            int CableCount = 0;
            middle = (min+max)/2;
            for (int i = 0; i < lanCable.length; i++) {
                CableCount += lanCable[i]/middle;
            }
            if(CableCount >= N) {
                min = middle+1;
            } else {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println(maxLength(lanCable, N));
            br.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
